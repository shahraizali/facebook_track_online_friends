...
def main():
    ...
    while True:
        interval_time = int(input(
            'How often would you like to check how many friends are online? Enter a number between 5 and 30 minutes: '),
                            10)
    ...
    while True:
        total_time = int(
            input('How long would you like to run this tool for? Enter a number between 1 and 720 hours: '), 10)
    ...
    if os.path.exists(path_to_csv_file):
        ...
        path_to_csv_file = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                        'facebook_online_friend_tracker_data.csv')
    ...
    while iteration < number_of_iterations:
        ...
        onlineFriendsCount = driver.find_element_by_xpath(
            '//*[@id="fbDockChatBuddylistNub"]/a/span[2]/span').text.strip('()')
        list_of_friends = []
        is_online = driver.find_element_by_xpath('//*[@class="_42fz"]').find_element_by_class_name("_568-").text
        # person is online
        # if is_online == "":
        #     pass
        friends = driver.find_elements_by_class_name('_42fz')
        for obj in friends:
            list_of_friends.append(obj.text)
        ...
        with open(path_to_csv_file, 'a') as f:
            writer = csv.writer(f, lineterminator='\n')
            writer.writerow([today, onlineFriendsCount, list_of_friends])
